-- @noindex
-- ReaScript Name: dda_RATING - Select all #2 color-rated items on selected tracks only
-- description: Selects all items on selected tracks that are colored with the color rating #2. Run the following script to set colors: dda_RATING - Show and change rating colors.lua
-- author: David Dumais
...
function readSecondCustomColor()
    ...
    if file then
        ...
        for line in file:lines() do
            count = count + 1
            if count == 2 then -- Second line for the second custom color
...
-- Function to select items with the second custom color on all selected tracks
function selectItemsWithSecondCustomColorOnSelectedTracks(r, g, b)
...
if r and g and b then
    ...
    -- Select items with the second custom color on all selected tracks
    selectItemsWithSecondCustomColorOnSelectedTracks(r, g, b)
    
    reaper.UpdateArrange() -- Update the arrangement to reflect the selection changes
    --reaper.ShowConsoleMsg("Items with the second custom color on all selected tracks have been selected.\n")
else
