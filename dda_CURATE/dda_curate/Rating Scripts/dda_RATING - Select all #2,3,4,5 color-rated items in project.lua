-- @noindex
-- ReaScript Name: dda_RATING - Select all #2,3,4,5 color-rated items in project
-- description: Selects all items in project that are colored with the color rating #2, #3, #4, or #5. Run the following script to set colors: dda_RATING - Show and change rating colors.lua
-- author: David Dumais
...
-- Function to read RGB values from the custom_colors.txt file for the second to fifth custom colors
function readSecondToFifthCustomColors()
...
function selectItemsWithCustomColor(r, g, b)
    for i = 0, reaper.CountMediaItems(0) - 1 do
        ...
        if item then
            ...
            -- Read RGB values for the second to fifth custom colors
            local customColors = readSecondToFifthCustomColors()
            if customColors then
                ...
                --reaper.ShowConsoleMsg("Items with the second, third, fourth, and fifth custom colors have been selected.\n")
            else
                --reaper.ShowConsoleMsg("Failed to read RGB values for the second to fifth custom colors.\n")
            end
