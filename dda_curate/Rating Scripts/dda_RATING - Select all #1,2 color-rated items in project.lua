-- @noindex
-- ReaScript Name: dda_RATING - Select all #1,2 color-rated items in project
-- description: Selects all items in project that are colored with the color rating #1 or #2. Run the following script to set colors: dda_RATING - Show and change rating colors.lua
-- author: David Dumais
...
-- Function to read RGB values from the custom_colors.txt file for the first and second custom colors
function readFirstAndSecondCustomColors()
...
function selectItemsWithCustomColor(r, g, b)
    for i = 0, reaper.CountMediaItems(0) - 1 do
        ...
        -- Read RGB values for the first and second custom colors
        local customColors = readFirstAndSecondCustomColors()
        if customColors then
            ...
            --reaper.ShowConsoleMsg("Items with the first and second custom colors have been selected.\n")
        else
            --reaper.ShowConsoleMsg("Failed to read RGB values for the first and second custom colors.\n")
        end
