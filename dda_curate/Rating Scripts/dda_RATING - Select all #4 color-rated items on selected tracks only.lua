...
function readFourthCustomColor()
    ...
    if file then
        ...
        for line in file:lines() do
            count = count + 1
            if count == 4 then -- Fourth line for the fourth custom color
...
-- Function to select items with the fourth custom color on all selected tracks
function selectItemsWithFourthCustomColorOnSelectedTracks(r, g, b)
...
-- Read RGB values for the fourth custom color
local r, g, b = readFourthCustomColor()
if r and g and b then
    -- Run the "Unselect all items/tracks/envelope points" command
    runActionByID(40289)
    
    -- Select items with the fourth custom color on all selected tracks
    selectItemsWithFourthCustomColorOnSelectedTracks(r, g, b)
    
    reaper.UpdateArrange() -- Update the arrangement to reflect the selection changes
    --reaper.ShowConsoleMsg("Items with the fourth custom color on all selected tracks have been selected.\n")
else
