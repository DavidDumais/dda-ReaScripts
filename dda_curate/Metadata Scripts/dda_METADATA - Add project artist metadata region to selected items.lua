-- @noindex
-- ReaScript Name: dda_METADATA - Add project artist metadata region to selected items
-- description: Adds a single region around all selected items and renames the region to user defined text. This region will be used to fill the artist metadata when exporting.
-- author: David Dumais
...
function createSingleRegionAndRename()
    ...
    local retval, userInput = reaper.GetUserInputs("Rename Region", 1, "Project Artist (name):,extrawidth=200", "")

    if retval then
        local regionName = "artist=" .. userInput
