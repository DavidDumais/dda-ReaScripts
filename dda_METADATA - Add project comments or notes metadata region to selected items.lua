-- @noindex
-- ReaScript Name: dda_METADATA - Add project comments or notes metadata region to selected items
-- description: Adds a single region around all selected items and renames the region to user defined text. This region will be used to fill the comment metadata when exporting.
-- author: David Dumais
...
function createSingleRegionAndRename()
    ...
    local retval, userInput = reaper.GetUserInputs("Rename Region", 1, "Project Comments/Notes:,extrawidth=200", "")

    -- Check if user input is not empty
    if retval and userInput ~= "" then
        local regionName = "comments=" .. userInput
